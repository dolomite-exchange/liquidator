...
export default class MarketStore {
  ...
  constructor(private readonly blockStore: BlockStore, private readonly filterOutBadPrices: boolean = true) {
    this.marketMap = {};
  }
  ...
  _update = async () => {
    ...
    const nextDolomiteMarkets = await Pageable.getPageableValues(async (lastId) => {
      const result = await getDolomiteMarkets(blockNumber, lastId, this.filterOutBadPrices);
